...
impl Worker {
    pub async fn do_work(self) {
        ...
        if raw_path.is_file() && self.url.starts_with("file:") {
            // Get parsed filename
            ...
            // Populate titles
            {
                ...
                titles[self.index] = Some(format!("Chapter {}", self.index + 1));
            }

            // Just copy it
            if let Err(e) = copy(raw_path, parsed_path) {
                log::error!(
