...
impl Parser for RanobesParser {
    fn parse_book_info(&self, url: &str, html: &str) -> Result<BookInfo, Error> {
        ...
        let text = meta.text().collect::<Vec<_>>().join("");
        let json: Value = serde_json::from_str(text.trim()).map_err(Error::json)?;
        let title = json["name"]
            .as_str()
            .ok_or(Error::json("no title in json"))?
            .trim();
        let author = json["author"]["name"]
            .as_str()
            .ok_or(Error::json("no author in json"))?
            .trim();
